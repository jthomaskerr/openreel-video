...
export class MemoryGenerationAudioCache implements GenerationAudioCache {
  ...
  get(key: string): PreparedGenerationAudio | undefined { return this.#values.get(key); }
  set(key: string, value: PreparedGenerationAudio): void { this.#values.set(key, value); }
}
...
export function buildGenerationAudioCacheKey(request: Readonly<GenerationAudioRequest>): string {
  const fields: Array<[string, string]> = [
    ['project', request.projectId], ['media', request.mediaId], ['version', request.versionId],
    ['projectStart', canonicalNumber(request.projectStartSeconds)], ['projectEnd', canonicalNumber(request.projectEndSeconds)],
    ['sourceStart', canonicalNumber(request.sourceStartSeconds)], ['sourceEnd', canonicalNumber(request.sourceEndSeconds)],
    ['sourceIn', canonicalNumber(request.sourceInSeconds)], ['trimStart', canonicalNumber(request.trimStartSeconds)],
    ['trimEnd', request.trimEndSeconds === undefined ? '' : canonicalNumber(request.trimEndSeconds)],
    ['speed', canonicalNumber(request.speed)], ['sampleRate', canonicalNumber(request.sampleRate)],
    ['channels', canonicalNumber(request.channels)], ['format', request.format],
  ];
...
const validateRequest = (request: Readonly<GenerationAudioRequest>): void => {
  const values = [request.projectStartSeconds, request.projectEndSeconds, request.sourceStartSeconds, request.sourceEndSeconds,
    request.sourceInSeconds, request.trimStartSeconds, request.speed, request.sampleRate, request.channels];
  if (request.trimEndSeconds !== undefined) values.push(request.trimEndSeconds);
  if (values.some(value => !Number.isFinite(value)) || request.projectStartSeconds < 0 || request.sourceStartSeconds < 0
    || request.sourceInSeconds < 0 || request.trimStartSeconds < 0 || request.projectEndSeconds <= request.projectStartSeconds
    || request.sourceEndSeconds <= request.sourceStartSeconds || request.speed <= 0 || !Number.isInteger(request.sampleRate)
    || request.sampleRate <= 0 || !Number.isInteger(request.channels) || request.channels <= 0
...
export function encodePcmS16leWav(samples: Int16Array, sampleRate: number, channels: number): Uint8Array {
  if (!Number.isInteger(sampleRate) || sampleRate <= 0 || !Number.isInteger(channels) || channels <= 0 || samples.length % channels !== 0) {
    throw new TypeError('audio-pcm-invalid');
  }
  const dataLength = samples.length * 2;
  const bytes = new Uint8Array(44 + dataLength);
  const view = new DataView(bytes.buffer);
  setAscii(view, 0, 'RIFF'); view.setUint32(4, 36 + dataLength, true); setAscii(view, 8, 'WAVE');
  setAscii(view, 12, 'fmt '); view.setUint32(16, 16, true); view.setUint16(20, 1, true);
  view.setUint16(22, channels, true); view.setUint32(24, sampleRate, true);
  view.setUint32(28, sampleRate * channels * 2, true); view.setUint16(32, channels * 2, true); view.setUint16(34, 16, true);
  setAscii(view, 36, 'data'); view.setUint32(40, dataLength, true);
...
export async function prepareGenerationAudio(input: {
  ...
  try {
    ...
    const prepared: PreparedGenerationAudio = {
      bytes, mimeType: 'audio/wav', sha256: await sha256(bytes), sampleRate: input.request.sampleRate,
      channels: input.request.channels, sampleCount: extracted.samples.length / input.request.channels,
      actualSourceStartSeconds: extracted.actualSourceStartSeconds, actualSourceEndSeconds: extracted.actualSourceEndSeconds,
      partial: extracted.actualSourceStartSeconds > input.request.sourceStartSeconds || extracted.actualSourceEndSeconds < input.request.sourceEndSeconds,
      cacheKey,
    };
    input.signal?.throwIfAborted();
