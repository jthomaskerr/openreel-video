...
export async function placeGeneratedAssetOnTimeline(
  ...
  if (policy === "none") {
    return { success: true, placed: false, trackId: "", clipId: "", status: "skipped" };
  }
  ...
  if (policy === "replace-selected-clip-media") {
    if (!input.clipId) {
      return {
        ...
        trackId: "",
        clipId: "",
        error: { code: "CLIP_NOT_FOUND", message: "A selected clip is required for replacement" },
      };
    }
    const target = findClip(store.project, input.clipId);
    if (!target) {
      return { success: false, placed: false, trackId: "", clipId: "", error: { code: "CLIP_NOT_FOUND", message: `Clip with ID ${input.clipId} not found` } };
    }
    if (input.shotId && target.clip.metadata?.shotId && target.clip.metadata.shotId !== input.shotId) {
      return { success: false, placed: false, trackId: target.track.id, clipId: target.clip.id, error: { code: "INVALID_PARAMS", message: "Selected clip does not belong to the requested shot" } };
    }
    if (!store.replaceClipMedia) {
      return { success: false, placed: false, trackId: target.track.id, clipId: target.clip.id, error: { code: "ACTION_FAILED", message: "Store does not support undoable clip replacement" } };
    }
    store.beginHistoryGroup?.("Replace generated clip media");
    try {
      const result = await store.replaceClipMedia(input.clipId, media.id, idempotencyKey);
      if (!result.success) return { success: false, placed: false, trackId: target.track.id, clipId: target.clip.id, error: result.error };
      return { success: true, placed: true, trackId: target.track.id, clipId: target.clip.id, status: "applied" };
    } finally {
  ...
  return { success: true, placed: true, trackId: trackResult.trackId, clipId: clip.id, status: "applied" };
}
...
function findClip(project: Project, clipId: string): { clip: Project["timeline"]["tracks"][number]["clips"][number]; track: Track } | null {
  for (const track of project.timeline.tracks) {
