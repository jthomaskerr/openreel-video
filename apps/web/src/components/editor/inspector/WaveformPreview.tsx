...
type WaveformPreviewItem = MediaItem & {
  ...
  waveformData?: Float32Array | number[] | null;
};
...
export function WaveformPreview({ item }: Props) {
  ...
  const waveformData = waveformItem.waveformData ?? null;
  ...
  }, [item.blob, item.id, item.metadata.duration, item.originalUrl, remoteUrl, waveformData]);
