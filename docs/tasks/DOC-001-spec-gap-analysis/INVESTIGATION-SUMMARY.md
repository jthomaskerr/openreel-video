...
1. **Type naming:** Should we standardize to `kieai` (lowercase) globally? Willing to migrate localStorage?
  ...
  ANSWER: Yes, standardize to `kieai`. Implement migration logic in settings store to handle existing `kie-ai` entries.
...
2. **Atlascloud API:** Has anyone verified the unknowns in spec §11?
   ...
   ANSWER: Do not directly call Atlascloud API endpoints. Use the appropriate modules from the atlascloud-cli package to handle requests and responses. This ensures compatibility with any future changes in the API.

3. **Priority:** Atlascloud only, or full 9 phases? (Affects timeline estimate)

   ANSWER: Full 9 phases.

4. **Multi-instance:** Is the requirement real (multiple API keys per provider) or can we defer to phase 9?

   ANSWER: No, multi-instance is only for LLM providers. Image/Video generators only require a single instance per provider. 
...
5. **Browser testing:** User-facing features (job panel, reference picker, default model) require browser verification before shipping. Budget for manual QA.
   ...
   ANSWER: OK, but utilise jsdom to test as much as possible in unit tests. Manual QA should be reserved for final verification of UI flows.
