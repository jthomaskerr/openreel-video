import { z } from "zod";
import { GENERATION_JOB_CONTRACT_VERSION, GENERATION_JOB_SCHEMA_VERSION, canonicalizeGenerationProjectActionPayload, serializeDurableGenerationValue, type GenerationJob, type GenerationOutput, type GenerationProjectActionCommand, type GenerationProjectActionResult, type GenerationProjectMutationReceipt, type GenerationShotAttempt, type SanitizedGenerationProvenance } from "./contracts.js";
...
export const JsonValueSchema: z.ZodType<unknown> = z.lazy(() => z.union([z.null(), z.boolean(), z.number().finite(), durableJsonString, z.array(JsonValueSchema), z.record(z.string(), JsonValueSchema)]));
...
export const ResolvedGenerationReferenceSchema = z.object({ id: strictIdentifier, order: z.number().int().positive(), mediaId: strictIdentifier, versionId: strictIdentifier.optional(), origins: z.array(GenerationReferenceOriginSchema).min(1), state: z.enum(["active", "failed"]), preparationStatus: GenerationPreparationStatusSchema, errorHistory: z.array(GenerationErrorSchema), uploadLeaseId: strictIdentifier.optional() }).strict().superRefine((v, c) => { if (v.state === "failed" && v.errorHistory.length === 0) c.addIssue({ code: "custom", message: "failed references require error history" }); });
